...
public abstract class RadiativeTransferSolver extends PropertyHolder 
															implements Reflexive , Describable {
	...
	public double getFluxMeanDerivativeFront() {
		double f = ( getFlux(0) - getFlux(1) ) 
				 + ( getStoredFlux(0) - getStoredFlux(1) );
		return f*_h;
	}
	
	public double getFluxMeanDerivativeRear() {
		double f = ( getFlux(N-1) - getFlux(N) ) 
				 + ( getStoredFlux(N-1) - getStoredFlux(N) );
		return f*_h;	
	}
