...
public class ExponentialFunctionIntegrator extends Integrator {
	...
	public double integrate(int order, double... params) {
		switch (method) {
		case MIDPOINT:
			return integrateMidpoint(order, params[0], 0, 1);
		case SIMPSON:
			return integrateSimpson(order, params[0], 0, 1);
		default:
			throw new IllegalStateException("Integration method undefined: " + method);
		}
	}
	...
	/*
	 * @Override public double integralAt(double t, int n) { if(t < 1E-10) return
	 * integralAtZero(n); else if(t < LOWER_CUTOFF) //use approximation return
	 * this.e_Approximation(n, t); else //use table return super.integralAt(t, n); }
	 */
	@Override
