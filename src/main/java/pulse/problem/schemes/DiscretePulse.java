...
public class DiscretePulse {
	...
	public double evaluateAt(double time) throws IllegalArgumentException {
		...
		case RECTANGULAR:
			return 0.5 * _WIDTH * (1 + signum(discretePulseWidth - time));
		case TRAPEZOIDAL:
			return 0.5 * _WIDTH * (1 + signum(discretePulseWidth - time)); // needs correction! TODO
		case TRIANGULAR:
			return _WIDTH * (1 + signum(discretePulseWidth - time))
					* (1 - abs(2. * time - discretePulseWidth) * _WIDTH);
		case GAUSSIAN:
			return _WIDTH * 5. / sqrt(PI) * exp(-25. * pow(time * _WIDTH - 0.5, 2));
		default:
			throw new IllegalArgumentException("Unknown pulse form received: " + pulse.getPulseShape().toString());
		}
	...
	public void optimise(Grid grid) {
		for (final double factor = 0.95; factor * grid.tau > discretePulseWidth; recalculate(
				NumericPropertyKeyword.PULSE_WIDTH)) {
			grid.tauFactor /= 1.5;
