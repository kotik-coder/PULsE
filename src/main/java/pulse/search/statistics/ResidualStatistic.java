 ...
 * The list of residuals is stored in a field value for objects of this class. Each {@code SearchTask} will have at 
 * least two {@code ResidualStatistic}s associated with its {@code Calculation}s.
...
public abstract class ResidualStatistic extends Statistic {
	...
	/**
	 * This will calculate the residuals for the {@code task} using the time sequence defined
	 * by the {@code ExperimentalData} object. The residuals are calculated between the model,
	 * which was previously used to populate the {@code HeatingCurve} and the experimental data.
	 * The temperature value of the model at the reference time is <math><msub><mi>t</mi><mi>i</mi></msub></math>.
	 * and unknown <i>a priori</i>. Therefore, it needs to be interpolated based on the discrete dataset
	 * generated by the solver. The interpolation is currently done using natural cubic splines, 
	 * which are re-constructed each time a new solution is generated. Therefore, calling this method
	 * does not involve expensive calculation of the spline coefficents. The residuals are calculated 
	 * only for the range that is specified by the {@code ExperimentalData} reference. The output of this method
	 * is stored in the field of the {@code residuals} object.  
