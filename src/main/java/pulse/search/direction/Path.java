 ...
 * which is related to a specific {@code SearchTask}. This information is used by the 
 * {@code PathSolver} to perform the next search step.</p>
 * 
 * <p>This is the most basic implementation, which stores only the gradient, the direction
 * (equal to the inverse gradient), minimum point achieved by the linear solver, and the number of
 * current iteration. It is used in combination with the {@code SteepestDescentSolver}. Note 
 * the constructors for {@code Path} are protected, as they should not be invoked directly. Instead,
 * they are invoked from within the {@code PathSolver}.</p>
 * 
 *
 */
...
public class Path {
	...
	/**
	 * Resets the {@code Path}: calculates the current gradient and the direction of search.
	 * Sets the minimum point to 0.0.
