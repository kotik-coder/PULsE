...
/**
 * An abstract class for baseline calculations. Defines an abstract {@code valueAt} method
 * that would return a baseline value at a given moment in time (either before or after 
 * the laser pulse). The baseline parameters can be modified within an optimisation loop,
 * hence there are two abstract methods to implement that functionality. 
 * 
...
public abstract class Baseline extends PropertyHolder implements Reflexive, Optimisable {
	...
	/**
	 * Calculates the baseline parameters based on input arguments. <p>
	 ...
	 * values. Upon completion, the method will use the respective {@code set} methods of this class to
	 * update the parameter values, triggering whatever
