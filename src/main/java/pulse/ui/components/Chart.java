...
public class Chart {
	...
	public static ChartPanel createEmptyPanel() {
		chart = ChartFactory.createScatterPlot("", Messages.getString("Charting.TimeAxisLabel"),
				(Messages.getString("Charting.TemperatureAxisLabel")), null, PlotOrientation.VERTICAL, true, true,
				false);
		...
		rendererOld.setSeriesStroke(0,
				new BasicStroke(2.0f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 2.0f, new float[] { 10f }, 0));
	...
	public static void plot(SearchTask task, boolean extendedCurve) {
		...
		Stroke dashed = new BasicStroke(1.5f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 5.0f, new float[] { 10f },
				0.0f);
		...
		if (zeroApproximationShown) {
			...
			if (p != null && s != null)
				plotSingle(HeatingCurve.classicSolution(p, (double) (s.getTimeLimit().getValue())));
		}
