...
public class IndexRange {
	...
	/**
	 * Construct an empty index range where the start index is set to -1 and the end
	 * index is set to 0.
	 */
	 ...
	 * bounded by its first and last elements, assuming that {@code data} is sorted
	 * in ascending order. Because of this last assumption, the visibility of this
	 * method has been set to protected.
	 * 
	 ...
	 * {@code closest(...)} to find an element in {@code data} either matching or
	 * being as close as possible to {@code a} (if {@code a} is non-negative) or
	 * zero.
	 * 
	 ...
	 * {@code closest(...)} to find an element in {@code data} either matching or
	 * being as close as possible to {@code b}. For the above operation, the list is
	 * searched through from its last to first element (i.e., in reverse order).
	 * 
	 * @param data the list to process
	...
	/**
	 * Gets the integer value representing the index of the lower bound previously
	 * set by looking at a certain unspecified data list.
	 * 
	...
	/**
	 * Gets the integer value representing the index of the upper bound previously
	 * set by looking at a certain unspecified data list.
	 * 
	...
	/**
	 * Searches through the elements contained in the the second argument of this
	 * method to find an element belonging to {@code in} most closely resembling the
	 * first argument. The search is completed once {@code of} lies between any two
	 * adjacent elements of {@code in}. The result is then the index of the
	 * preceding element.
	 * 
	 ...
	 * Searches through the elements contained in the the second argument of this
	 * method to find an element belonging to {@code in} most closely resembling the
	 * first argument. The search utilises a reverse order, i.e. it starts from the
	 * last element and goes to the first. The search is completed once {@code of}
	 * lies between any two adjacent elements of {@code in}. The result is then the
	 * index of the preceding element.
	 * 
	 * @param of an element which will be compared against
	 * @param in a list of data presumably containing an element similar to
	 *           {@code of}
